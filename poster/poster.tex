...
(ii) As $N \to \infty$, $p(t)  \to p(B=b)$, where $B$ is the subset of $k$ variables in the branch leading to $t$ and $b$ the vector of values of these variables ;\\
(iii) As $N_T \to \infty$, all branches $B=b$ of size $k$ appear with probability $\frac{1}{C_p^k}$ and $X_m$ is tested at the end of $\frac{1}{p-k}$ of them.
...
\textbf{Task.} Let us consider a 7-segment indicator (Fig. (a)) displaying numerals
using lights in on-off combinations. Let $Y$ ba random variable taking its value in ${0, 1, ..., 9}$
